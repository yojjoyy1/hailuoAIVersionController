...
目前這個專案設定的 AI Agent：**Claude**
你正在 Claude Code。`--source` 填目前對話名稱。專案 skill 在 `.claude/skills/avc-shiguangben/`，可用 `/avc-shiguangben`。
...
1. 確認已 `init`（有 `.avc/avc.sqlite`）。沒有就 `init --name "..."`，`--agent claude`。
2. 先 `status`，看回傳的 `active_session_id`：**若是 null（沒有進行中的對話），一定要先** `session start --title "<這次要做的事>" --source "<目前對話名稱>" --agent claude`。
...
```text
AVC init --name "專案名稱" --agent claude
AVC status
AVC session start --title "改封面" --source "對話名稱" --agent claude
AVC session end
